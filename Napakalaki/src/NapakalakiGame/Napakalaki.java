...
actualizar currentPlayer. */
    private Player nextPlayer() {
        ...
        if(currentPlayer == null) {
            ...
            do numero = (numero +1) % players.size();
            while(players.get(numero) != currentPlayer); 
        }
...
en cuenta que un jugador no puede ser enemigo de sí mismo. */
    private void setEnemies () {
        ...
        for(Player jugador : players) {
            do{
                contador = Dice.getInstance().nextNumber() % this.players.size();
            } while(players.indexOf(jugador) == contador);
            jugador.setEnemy(this.players.get(contador));
        }
    }
...
/*
////////    SINGLETON   /////////
///// Clase Singleton sin parámetros /////
    
public class Singleton {
    // me declaro la instancia(se puede llamar instance o como sea)
    private static Singleton instance = null;
    // el constructor vacío a no ser que haya parámetros
    private Singleton() {}
    // el método que controla para que no haya más de una instancia.
...
///// Clase Singleton con parámetros /////
    
public class Singleton {
    // me declaro la instancia(se puede llamar instance o como sea)
    private static Singleton instance = null;
    // parámetros que tendrá nuestro objeto
    private String dni;
    private int numero;
    // el constructor ahora si que tiene que tener sus parámetros
    private Singleton(String dni, int numero) {
        this.dni = dni;
        this.numero = numero;
    }
    // el método que controla para que no haya más de una instancia.
    public static Singleton getInstance(String dni, int numero) {
        if(instance == null) {
            instance = new Singleton(dni, numero);
        }
        return  instance;
    }
